...
public class Board3D extends AbstractBoard<Coordinate3D> {
    ...
    private boolean testOverRange(Function<Integer, Integer> fI, Function<Integer, Integer> fJ, Function<Integer, Integer> fK, Player player) {
        if (this.testOverRange(x -> this.cells[fI.apply(x)][fJ.apply(x)][fK.apply(x)] == player)) {
            this.winning = IntStream.range(0, this.getSize())
                    .mapToObj(x -> new Coordinate3D(fI.apply(x), fJ.apply(x), fK.apply(x)))
                    .toArray(Coordinate3D[]::new);
            return true;
        }
        return false;
    }
